...
基于 **MediaPipe Hands** 的实时手势识别，结合 **YOLO-World** 物品检测、**Kimi / Moonshot** 大模型对话与 **ASR/TTS** 语音，构建了一套面向无障碍沟通的多模态桌面系统。使用 **PySide6** 打造深色仪表盘界面，支持双手同时识别、静态 + 动态手势、手势方向检测，并在「💬 Kimi 对话 / 🎯 实时监测 / ⚙ 系统设置 / 🎮 小游戏」四个标签页中完成识别 → 对话 → 语音播报的完整闭环；内置可扩展的「切水果」摄像头小游戏，支持 简单 / 普通 / 困难 / 地狱 四档难度。
...
| **五标签页 UI** | 💬 Kimi 对话 / 🎯 实时监测（手势含义）/ 📜 识别历史 / ⚙ 系统设置 / 🎮 小游戏 |
...
| **👤 账号管理** | 系统设置底部「👤 账号操作」分组：**🚪 退出登录**（二次确认后返回登录界面，账号数据保留，程序不退出）；**🗑️ 注销用户**（二次警告确认后，删除该账号数据库记录 + 对应人脸图片 `faces/face_<编号>_*.jpg` + 重新训练识别模型，不可恢复，随后返回登录界面） |
| **识别历史（会话内）** | 识别结果实时写入 SQLite（logs 表），可在「⚙ 系统设置 → 数据记录」导出为 CSV；**关闭程序自动清空**，仅保留本会话记录 |
...
| 右侧（Tab 3 ⚙） | 系统设置：开关（骨架 / 镜像）、数据记录（截图 / 保存 / 导出识别历史 CSV）、API 与配置（Key 输入，保存时自动检测）、👤 账号操作（退出登录 / 注销用户） |
...
**Q: 想换人脸 / 删账号？**  
A: 在「⚙ 系统设置 → 👤 账号操作 → 🗑️ 注销用户」可图形化删除当前账号及其全部人脸数据（二次警告确认，不可恢复），注销后自动返回登录界面。若想彻底重置全部账号，可直接删除 `auth.db`；或手动删除 `faces/face_<编号>_*.jpg` 并删除 `trainer.yml` 后重新注册以重建识别模型。
