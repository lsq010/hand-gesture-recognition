...
基于 **MediaPipe Hands** 的实时手势识别，结合 **YOLO-World** 物品检测、**Kimi / Moonshot** 大模型润色与 **ASR/TTS** 语音，构建了一套面向无障碍沟通的多模态桌面系统。使用 **PySide6** 打造深色仪表盘界面，支持双手同时识别、静态 + 动态手势、手势方向检测，并在「💬 Kimi 对话 / 🎯 实时监测 / 📜 识别历史 / ⚙ 系统设置」四个标签页中完成识别 → 翻译 → 语音播报的完整闭环。
...
| **后台初始化** | 点击「打开摄像头」后所有重模型在后台线程加载，界面立即响应，不再冻结 5 秒 |
...
> **画圈 vs 挥手如何区分**：旧版仅靠「围合面积」单一特征，容易互相误判。现改用**绕质心累计有符号转角 + 等周比**双特征——画圈转角接近 ±360°、等周比高；挥手来回摆动、转角≈0、等周比≈0，两者几乎不再混淆。画面上轨迹线也按手势染色，肉眼即可区分。
...
- **API Key 安全**：Key 只保存在本次会话内存，**不会**写入数据库或读取环境变量，重启程序后必须重新粘贴输入（避免 Key 泄露后被静默复用）。可在「⚙ 系统设置 → API 与配置」中点击「🔍 检测 Key 是否可用」验证 Key 有效性。

## 界面布局

| 区域 | 说明 |
|------|------|
| 左侧 | 摄像头画面（实时叠加骨架、掌心轨迹、动态手势染色）；关闭摄像头后回到纯黑 |
...
| 右侧（Tab 4 ⚙） | 系统设置：开关（骨架 / 镜像）、数据记录（截图 / 导出）、API 与配置（Key 输入 + 检测） |
...
├── docs/                    # 演示 GIF 等文档资源
├── main.py                  # 主程序（PySide6 GUI 控制器 / 程序入口，含 CameraInitWorker 后台初始化线程）
...
├── sequence_tracker.py      # 动态手语轨迹追踪（Wave/Circle/Swipe，转角+等周比判定）
...
- **摄像头初始化**：点击「打开摄像头」后在 `CameraInitWorker(QThread)` 后台线程中完成 VisionEngine / YOLO / whisper / TTS / Kimi 的全部加载并预热 2 帧，UI 主线程不再冻结（消除旧版约 5 秒卡顿）；完成后经 `done` 信号回主线程启动实时循环。
...
A: 在「⚙ 系统设置 → API 与配置」粘贴 Moonshot API Key 并点「保存配置」，再用「🔍 检测 Key 是否可用」按钮验证。注意 **Key 仅本次会话有效，重启程序后必须重新输入**（不再自动从环境变量 / 数据库读取，以防泄露 Key 被静默复用）。

**Q: 物体检测无反应？**  
A: 确认根目录有 `yolov8s-world.pt`，且首次运行可联网（会自动下载 CLIP 权重到 `weights/clip/`）。

**Q: 点击「打开摄像头」后界面卡住好几秒？**  
A: 已优化为后台线程初始化——点击后状态栏立即提示"正在初始化多模态组件…（后台加载中）"，重型模型在后台加载，完成后画面自动出现，不再冻结界面。
...
- [ ] **性能与模型优化**：引入轻量级自定义分类模型，降低 CPU / GPU 占用；whisper 可改为首次语音输入时才加载。
