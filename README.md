...
如果你想把程序打包成一个**双击即用的 exe**（目标机器无需装 Python / 依赖），用项目内已配好的 PyInstaller 配置即可：

1. **准备运行环境**（在本机「装齐全部依赖」的 Python 下，例如 `D:\Desk\test\v1` 或系统 Python 3.10）：
   ```bash
   pip install pyinstaller
   pip install -r requirements.txt
   ```
2. **确保模型权重已就绪**（内置离线需要）：首次运行 `python main.py` 并打开「🎯 实时监测」标签页，会自动下载 `yolov8s-world.pt`（项目根）与 `weights/clip/`（CLIP 权重）。若已存在则跳过。
3. **执行打包**：
   ```bash
   pyinstaller build_exe.spec
   ```
4. **产物**：`dist/手势无障碍交流系统/` 整个文件夹。**双击其中的 `手势无障碍交流系统.exe`** 即可运行，无需联网（模型已内置）。

> 说明：采用**单文件夹**形态（`build_exe.spec` 用 `COLLECT`），启动稳定、坑少；GUI 程序设 `console=False`，不弹黑窗口。若某个资源文件缺失，打包脚本会打印 `[warn]` 并跳过，运行时该资源会退化为联网下载。人脸数据（`faces/`、`auth.db`、`trainer.yml`）不会被打包进 exe，首次运行会在 exe 同目录自动生成。
