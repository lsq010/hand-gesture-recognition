...
| **🔐 登录 / 注册入口** | 程序启动时先弹出登录窗口；支持**账号密码登录**与**人脸登录**（Haar 检测 + LBPH 识别，模型来自 `D:\Desk\test\haar`）；注册需填写账号 / 密码 / 确认密码 / 人脸编号并录入人脸（≥5 张），密码以 pbkdf2 加盐哈希存储；未注册账号登录提示「登录失败，请注册」 |
...
- **启动即登录**：运行 `python main.py` 后，首先弹出 **登录 / 注册** 窗口。未注册过账号时，请先切到「注册」页：填写账号 / 密码 / 确认密码 / **人脸编号**，点「📷 录入人脸」对着摄像头拍 ≥5 张（提示「保存成功」），再点「✅ 注册」完成（提示「注册成功」）。之后每次启动可用**密码登录**或**人脸登录**。用未注册账号密码登录会提示「登录失败，请注册」。登录成功后才进入主程序界面。
...
**Q: 人脸登录识别不出来 / 总失败？**  
A: 人脸登录依赖 `D:\Desk\test\haar` 下的 Haar 级联检测 + LBPH 识别，置信度阈值 `CONF_THRESHOLD=70`（越低越像）。请保证注册与登录时光线、角度一致，且录入时拍够 5~15 张、人脸居中清晰。若仍失败，可改用密码登录，或在 `auth.py` 适当调高阈值（放宽匹配）。
